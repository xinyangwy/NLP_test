...
if __name__ == '__main__':
    ...
    st.title('实验三：中文分词技术应用')
    st.write("作者:武梓龙 2019212300")
    sentence = st.text_input('请输入一段话','他是研究生物的')

    FMM_results = []
    BMM_results = []
    FMM_results.append(FMM(sentence))
    BMM_results.append(BMM(sentence))

    if st.button('显示分词结果'):
        st.write('FMM分词结果：', str(FMM_results))
    # if st.button('BMM'):
        st.write('BMM分词结果：', str(BMM_results))
    # else:
    #     st.error('您输入有误！')
